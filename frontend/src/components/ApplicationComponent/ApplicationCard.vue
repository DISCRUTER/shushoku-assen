...
async function fetchApplicationData() {
    try {
        const response = await apiClient.get(`/api/v1/applications/${props.applicationId}`);
...
function handleStatusUpdate(data) {
    applicationData.value.status = data.status;
}
    ...
    <div class="student-card" @click="showInfo=true">
        ...
        <template v-if="userRole==='Student'">
            <div class="student-about">
                {{ applicationData.status }}<br>
                {{ applicationData.drive?.company?.registered_name }}
            </div>
            <div class="drive-name">
                <h3>{{ applicationData.drive?.title }}</h3>
            </div>
        </template>
        <template v-else>
            <div class="student-about">
                {{ applicationData.status }}<br>
                {{ applicationData.student?.first_name }} {{ applicationData.student?.last_name }}
            </div>
            <div class="drive-name">
                <h3>{{ applicationData.drive?.title }}</h3>
            </div>
        ...
        <Teleport to="body">
			<ApplicationInfo :show="showInfo" @close="showInfo = false" :application-data="applicationData" @statusUpdated="handleStatusUpdate" />
		</Teleport>
    </div>
</template>
